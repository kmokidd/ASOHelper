...
function parsingData(fileUrls) {
  ...
  fileUrls.forEach(fileUrl => {
    ...
    while(desiredWordCell) {
      ...
      newComer.data = linkRecord(rowIndex, worksheet);
  ...
  // the word
  function linkRecord(index, worksheet) {
...
function sortResult(arr, type, order) {
  arr.sort((prev, next) => {
    if(order < 0) // 从小到大排列
...
module.exports = exports = (fileUrls) => {
  console.log("in mod analyse compete");

  let resultArr = parsingData(fileUrls);
  sortResult(resultArr, TYPE_COUNT, -1);
